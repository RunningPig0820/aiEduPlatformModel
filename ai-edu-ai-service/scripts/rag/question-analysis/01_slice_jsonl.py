"""
question-analysis 分库 jsonl - 切片数据/**/切片/*.md → rag_slices-question-analysis.jsonl
...
用法: cd ai-edu-ai-service && venv/bin/python scripts/rag/question-analysis/01_slice_jsonl.py
输入: docs/rag/question-analysis/切片数据/**/切片/*.md
输出: scripts/rag/data/rag_slices-question-analysis.jsonl(覆盖旧 90 条实验版)
"""
import glob
import json
import logging
...
OUT = os.path.join(os.path.dirname(__file__), "..", "data", "rag_slices-question-analysis.jsonl")
MIN_CHARS = 60
...
def _section_from_filename(name: str) -> str:
    """格式A: 文件名入口段切前缀 → 源文档名(引导问题→引导问题, 语雀-方案总揽-1-*→语雀-方案总揽)。"""
...
def parse_slice(path: str) -> dict | None:
    ...
    for ln in lines:
        ...
        if not header_done:
            ...
            if s.startswith(">") and not s.startswith("---"):
                # 头字段通用解析: 拆 ｜ 分段逐 key 取值(覆盖 单行 权威度/模块/... 与
                # 合并行 权威度｜来源｜锚点 / 来源｜锚点 / 模块｜节)
    ...
    # source ← 路径顶层目录(切片数据/{语雀|代码|坑档案|引导问题|OpenSpec}/切片/...)
    ...
    block = {
        ...
        "tags": {
            ...
            "source": source,
            "authority": float(meta["authority"]) if meta.get("authority") else 0.8,
