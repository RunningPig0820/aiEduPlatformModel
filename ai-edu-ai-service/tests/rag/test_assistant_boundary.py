...
- 双路置信度都低于阈值(vec<0.72 且 bm<0.5) → boundary
- 单路达阈值即通过(双路互补, 一路够用)
...
class TestCheckBoundary:
    ...
    def test_boundary_threshold_exact(self):
        """阈值边界: vec=0.72 过 / 0.71 拒(bm 低); bm=0.5 过 / 0.49 拒(vec 低)"""
        # 0.71 < 0.72 且 0.3 < 0.5 → 拒
        assert assistant.check_boundary(RERANK_HIT, vec_conf=0.71, bm_conf=0.3) is not None
        # 0.72 ≥ 0.72 → 过(单路够)
        assert assistant.check_boundary(RERANK_HIT, vec_conf=0.72, bm_conf=0.3) is None
        # 0.49 < 0.5 且 0.0 < 0.72 → 拒
