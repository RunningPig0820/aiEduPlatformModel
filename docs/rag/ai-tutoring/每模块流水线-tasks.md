...
- [x] 1.3a **对齐 `1.语雀` 5 篇**：逐篇标注 `保留 / 过时(修正为代码真相) / 演进故事(规划未落地)`。
...
- [ ] 1.3b **对齐 `2.OpenSpec` 30 文件**：逐份（design+proposal）对照代码，标注 `一致 / 有改动点`。
      已知改动点：`design-backend-ai-tutoring` 决策8/9 掌握度已翻转（知识点→题型）｜OCR"反转"实为双通道 ｜"学生确认知识点"实为低置信澄清卡
- [ ] 1.3c **对齐 `3.代码` 摘录**：代码即真相，与 OpenSpec 方案逐项核对，产出差异明细（含代码位置）
- [x] 1.3d **合成 `方案-代码对账.md`**（差异汇总 + 每处"方案写了什么 / 代码实际怎样 / 完善文档怎么写"）
      > **初版已产出**（前端入口→后端代码→对照方案，圈出 3 处口径差异）；1.3a 语雀对齐已并入 1.语雀 各篇。待补：1.3b OpenSpec 逐份 + 1.3c 代码引用后回填对账
