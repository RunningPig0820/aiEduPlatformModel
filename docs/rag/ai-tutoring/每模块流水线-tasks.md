...
- [ ] 8 节齐全、非空模板（完整性检查）——文件已齐（8/8），待 1.7 统一完整性核对
- [ ] **6.缺失补充**（2026-08-24 新增）：逻辑闭环缺口 8 个（行为风控/题库/意图分类/掌握度翻转/护栏归属/不用agent/知识点确认/闭环边界）落盘 `6.缺失补充/逻辑闭环缺口-待回答.md`，用户填写后回填对应节加「追问与防御」

### 1.5 切片（认真设计，非"按标题切一刀"）
- [x] **切片清单**（2026-08-24）：`切片清单.md` 定稿——**四层全进 + 权威度分层**（完善文档1.0 / 语雀+OpenSpec 0.7 / 代码分析+坑档案 0.8）；OpenSpec 22 文件精选（design 11 全进 + proposal 核心 4 进，其余 7 不进）；坑档案实际路径 `5.难点/坑档案.md`
