...
> summary: 汇总知识图谱模块全部设计决策，记录每一项决策的背景、演进过程、拍板理由与系统影响（Neo4j 选型/方案C 数据加工/URI v3.1/bge 512+dashscope 768 双 embedding/匹配双模型投票/前置依赖融合/llmTaskLock 断点续传/页面化方案 B/双数据源/URI 主键）
> 权威度: 0.8
> 模块: knowledge-graph
> COS路径: rag-source/knowledge-graph/语雀/语雀-决策记录.md
> 类别：架构设计

> 来源：docs/rag/knowledge-graph/1.语雀/原来的文件/（36 份）+ edukg 代码真值 + 2.OpenSpec design 决策 13 份。状态口径：✅=已落地/已拍板；⚠️=建议项(设计构想未落地)；❓=待决。
