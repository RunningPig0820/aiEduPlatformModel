> summary: 知识图谱独立数据库 ai_edu_kg 双数据源架构设计，解决 Spring Boot 单数据源无法物理隔离问题，选用 Baomidou dynamic-datasource 多数据源 starter，通过 @DS 注解按 Mapper 包路径路由，事务按数据源隔离，Flyway 迁移脚本按库分组，对现有业务零侵入。
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-source/knowledge-graph/OpenSpec设计决策/design-backend-2026-06-03-knowledge-graph-datasource.md
> 类别：架构设计

# design-backend-2026-06-03-knowledge-graph-datasource（知识图谱数据源同步后端）
