> summary: 本文档为Java后端知识图谱UI数据方案：用户选型方案B，将Neo4j教材知识点（Textbook/Chapter/Section/TextbookKP）同步到MySQL供前端SPA读取，图谱关系（MATCHES_KG/PART_OF/RELATED_TO）不同步、直接查Neo4j。核心决策覆盖同步策略（手动按需+UPSERT+状态机+单事务）、URI主键、6级导航树、API设计与DTO、下拉选项枚举+MySQL混合、Neo4j降级缓存。
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-source/knowledge-graph/OpenSpec设计决策/design-backend-2026-06-03-knowledge-graph-ui.md
> 类别：架构设计

# Java 后端知识图谱 UI 数据方案（Neo4j→MySQL 同步 + 导航 + 知识体系）
