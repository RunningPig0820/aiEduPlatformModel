> summary: kp-matching-lightup 技术设计：AI 答疑题型解析到教材知识点 URI 的派生层方案，掌握度主体从知识点翻转为题型，派生层 3 表（t_kp_derived_obs/t_kp_question_type/t_kp_question_type_kp）只存 MySQL、权威图谱 Neo4j 零写入，LLM 消歧+学生澄清+人工仲裁三方信任模型，自动维护闭环与冷启动弱化防 LLM 幻觉。
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-source/knowledge-graph/OpenSpec设计决策/design-backend-kp-matching-lightup.md
> 类别：数据存储
