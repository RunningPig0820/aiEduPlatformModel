> summary: 学生端知识点匹配点亮前端设计，核心模型纠正为掌握度主体=题型而非知识点、知识点由 QuestionTypeKp ratio 派生；落地学习报告总纲+掌握度/知识点总览/题型分析三视图、KnowledgeGraph 叠加 masteryMap 着色、答疑澄清卡 resolve+vote 两步接口。
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-source/knowledge-graph/OpenSpec设计决策/design-frontend-kp-matching-lightup-frontend.md
> 类别：业务流程

# kp-matching-lightup-frontend 技术设计（知识点点亮前端）
...
> 状态：
> 检索摘要：管理端审核队列本期前端不做（后端接口已就绪），后续挂 /admin/knowledge-graph 图谱页，教师端同理不做。
...
### Open Questions
> 状态：
> 检索摘要：Open Question 已关闭：题型分析并入掌握度页；派生覆盖度契约后端同时返回 coverage 0-75 与离散四档 masteryLevel。
