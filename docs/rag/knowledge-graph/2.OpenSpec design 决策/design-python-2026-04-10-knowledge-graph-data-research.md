> summary: 知识图谱数据整理与前置依赖构建方案：Neo4j 知识点/关系数据模型、教材年级推断、多证据融合(定义依赖+GLM/DeepSeek多模型投票)构建 PREREQUISITE 学习依赖、分学科处理策略与工程化容错(MySQL状态管理/断点续传/成本控制)。
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-source/knowledge-graph/OpenSpec设计决策/design-python-2026-04-10-knowledge-graph-data-research.md
> 类别：数据关联

# 知识图谱数据整理方案设计（RAG 结构化重构）
