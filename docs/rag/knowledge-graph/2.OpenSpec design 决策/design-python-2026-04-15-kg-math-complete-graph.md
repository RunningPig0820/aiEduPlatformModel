> summary: 本文档为数学知识图谱完整数据管道设计：解析教材JSON生成节点关系、LLM双模型推断补全教学知识点、向量检索+双模型投票将TextbookKP匹配到EduKG Concept（MATCHES_KG），并覆盖断点续传、数据清洗、知识点属性扩展、URI v3.1规范与迁移计划。核心选型：BAAI/bge-small-zh-v1.5粗筛top-20候选、difflib对比、置信度阈值0.7/0.9。
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-source/knowledge-graph/OpenSpec设计决策/design-python-2026-04-15-kg-math-complete-graph.md
> 类别：操作流程

# 数学知识图谱完整数据管道设计（kg-math-complete-graph）
