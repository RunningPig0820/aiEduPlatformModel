...
> summary: 解答「教材知识点怎么匹配进图谱、匹配率怎么从 17% 提到 97%」——标准化精确匹配(同义词+best_match) → 向量粗筛 top-20(bge 或 difflib) → 双模型投票(GLm-4-flash+DeepSeek, 一致采纳/不一致加权 DS=0.6 GLM=0.4 阈值 0.5, DS 否决) → 未匹配进 candidate_review, LLM 调用三重优化(相似度<0.5 跳过+候选上限3+早停) 从 40→2 次/知识点。
> 权威度: 0.8
> 模块: knowledge-graph
...
| 方案vs实现 | 匹配率 | 语雀 1690/1740=97.1%（早期里程碑） | 当前文件 1847/1905=96.96%（后续 merge 后批次） | ⚠️翻转（口径不同批次，非错误） |
| 方案vs实现 | 未匹配数量 | 语雀 308（297 投票不通过+11 无候选） | 当前 58（candidate_review 50+none 8） | ⚠️翻转（不同批次） |
| 方案vs实现 | D16 评审系统 | MySQL 审核表+/api/kp-match/recommend+导入脚本 | `edukg/scripts` 与 `ai-service/api` 无对应代码；仅 candidate_review 记录 | ⚠️方案有/代码无 |
| 接口契约 | 缓存键 | 语雀称 `md5(textbook_kp+kg_kp)` | 实际 `SHA256(prompt)[:16]`（llm_cache.py:30），Prompt 含名称+描述+学段 | ⚠️翻转 |
| 注释vs运行行为 | 三重优化 | 语雀"40→2 次 API/知识点" | LLM_CANDIDATE_LIMIT=3+0.5 阈值+早停逻辑全落地 | ✅ 落地 |
| 注释vs运行行为 | vote_match/vote_prerequisite | 模块文档列为匹配/前置投票规则 | 两方法定义但主流程未调用（主流程走 `vote()`→`_check_consensus`） | ⚠️死代码 |
