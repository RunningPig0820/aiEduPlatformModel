...
> summary: 解答「知识点向量索引怎么建、怎么保证不过期」——匹配侧 bge-small-zh-v1.5 本地 512 维建 kg_vectors.npy(1295×512)+index_meta.json(MD5 checksum 校验), 懒加载 3.5GB vs 预构建 ~10MB 两种检索器; 服务侧 dashscope text-embedding-v3 768 维写 COS 向量桶(topic/RAG 双池, put 后~10s 异步生效)。
> 权威度: 0.8
> 模块: knowledge-graph
...
| 方案vs实现 | 预构建大小 | 语雀/注释约 10MB | 实际索引文件约 3.1MB（npy 2.6MB+json） | ⚠️翻转（注释"10MB"偏大，可能含内存口径） |
