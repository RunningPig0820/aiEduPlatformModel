...
> summary: 决策：手动按需触发同步（非CDC），按URI UPSERT，整个同步在单事务内保证原子性；状态机active/deleted/merged，查询过滤deleted但进度查询例外。
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-slices/knowledge-graph/OpenSpec/design-backend-2026-06-03-knowledge-graph-ui-D2-同步策略-按需触发-upsert-状态机.md
> 类别：操作流程
...
> 证据：详见 `2.OpenSpec design 决策/design-backend-2026-06-03-knowledge-graph-ui.md`（§D2：同步策略：按需触发 + UPSERT + 状态机）
