...
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-slices/knowledge-graph/OpenSpec/design-kp-lightup-backend-11-三端架构与模块分工.md
> 类别：架构设计

---

> 检索摘要：方案按 DDD 域分工——派生层业务落 learning 域（domain/infrastructure/application/interface 四层），解析管线在 infrastructure/ai/tutoring 集成层，权威图谱归 edukg 域只读；在线逻辑（解析管线/掌握度点亮）与离线逻辑（obs→题型库聚合/维护重判/先验漂移）拆分，离线进 batch 包标注大数据归宿。

**模块归属（DDD 域定位）**：方案核心业务落在 learning 域；答疑入口与权威图谱边界如下：
...
**D11 在线 vs 离线边界：大数据逻辑单独隔离**：派生层逻辑按「实时在线」与「离线批处理」拆分：
...
离线逻辑单独拆到 batch 包，package-info.java + 类 javadoc 明确标注「逻辑归宿=大数据平台，当前后端 @Scheduled 过渡实现」。理由：聚合/维护本质是离线批处理（不要求实时、obs 无限长尾），理想归宿是大数据平台；当前项目纯 Java DDD 后端未接大数据，故先以 @Scheduled 过渡。数据表（obs/题型库）为中性结构，大数据可直接读写，未来迁移只需替换 batch 包，在线解析管线②与数据表不变。
