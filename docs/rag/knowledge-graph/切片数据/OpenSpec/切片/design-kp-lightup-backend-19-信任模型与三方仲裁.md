...
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-slices/knowledge-graph/OpenSpec/design-kp-lightup-backend-19-信任模型与三方仲裁.md
> 类别：架构设计

---

> 检索摘要：派生层「谁是最终裁判」按各擅长的分工——LLM 规模化主裁判、学生意图信号源、人工仅边界仲裁；信任度=准确率×可复现性×可规模化×可治理性；LLM 每次判断须被客观信号校准防自证循环；学生澄清只碰学科概念不碰 kp_uri，≥3 人一致才进候选。

**D8 信任模型：LLM 主裁判 + 学生意图信号 + 人工边界仲裁**：派生层需要持续维护，但「谁是最终裁判」不能单一押注 LLM / 人工 / 学生任一方。三者各有不可替代的位置，按「各做其最擅长的事」分工：
