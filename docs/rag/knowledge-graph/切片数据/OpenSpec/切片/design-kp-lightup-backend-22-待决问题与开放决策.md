...
> 权威度: 0.7
> 模块: knowledge-graph
> COS路径: rag-slices/knowledge-graph/OpenSpec/design-kp-lightup-backend-22-待决问题与开放决策.md
> 类别：未来演进

---

> 检索摘要：8 项待决——Python mastery_signals 是否改名 topic_label、student_grade 来源、LLM 消歧调用方式、聚合阈值初值、掌握度自动迁移、消费方、澄清卡数据契约、topic_key 归一化力度。

0. 【高优先级·跨仓库协调】Python mastery_signals 信号源粒度：现状 kp_label 是自由文本（题型/知识点混合），要翻题型粒度需 Python 稳定输出题型 label。字段名 kp_label 是否重命名为 topic_label？（默认：建议重命名 topic_label 语义清晰，Java MasterySignalItem 对应改 @JsonProperty；Python 未就绪前 Java 侧兼容旧字段名 kp_label 作为过渡）。
