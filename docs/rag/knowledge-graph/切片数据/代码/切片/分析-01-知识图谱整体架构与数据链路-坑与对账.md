...
- **api/kg.py 端点数口径**：原始方案总揽 §10 称"八端点"，实际落地 api/kg.py 实为 10 个路由（含 progress GET/POST 两态）。⚠️翻转——口径"八端点"少算，实 10 路由。
...
- **服务查询标签**：原始方案总揽 §4 数据模型用具体标签，实际落地 service.py 查询统一 `:Entity`，import 用具体标签，标签体系可能不匹配。⚠️翻转——需联调核对。
- **/query 读写防护**：原始方案总揽未特别声明读写，实际落地 read_only 默认 true 但调用方可关。✅落地——有防护缺约束。
- **知识树/学习路径**：原始方案页面化期望真层级/前置链，实际落地 Python 桥为简化桩（平铺/单点）。⚠️翻转——真实能力在 Java/MySQL 侧。
