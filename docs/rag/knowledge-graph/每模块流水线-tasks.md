...
- [ ] 代码深读（Explore agent，多端一起读）→ `3.代码/分析-01~10.md`（落地真相 + 文件:行号 + 业务描述与业务场景先行）
- [ ] 方案 vs 代码 → `方案-代码对账.md`（候选翻转：年级来源/匹配实现/GraphRAG 范围/页面化消费/前置权重/断点续传）
- [ ] `5.难点/坑档案.md`（J-KG#，真挖多端 git log 第一铁律 + git commit + 文件:行号 双证据）

## 3. 完善文档 9 节（⏳）

- [ ] `4.完善文档/01~09.md`（权威 1.0 前置校验：方案素材 + 代码真值缺一降 0.8；整篇禁切；文件头 5 行标准）
- [ ] 差异区厚写：05 推断匹配（阈值/投票权重）、06 前置依赖（三来源权重）、09 业务闭环（端到端全景 + GraphRAG 落地）

## 4. 切片（⏳）

- [ ] 代码层双切片：`切片-开发向-提示词.md` + `切片-分组-提示词.md` → 60 块
- [ ] 坑档案双切片：`坑档案-切片-提示词.md` → 一坑一文件
- [ ] 引导问题问答切片：`引导问题-切片-提示词.md` → 一问题一文件
- [ ] 语雀按条目切：`语雀-切片-提示词.md` → 决策/选型/场景/阶段各一条一文件
- [ ] OpenSpec 按 ### 切：`OpenSpec-切片-提示词.md` → design 素材（0.7）
- [ ] 切片清单核对块数、类别闭集、头部精简 5 行

## 5. 引导问题（⏳）

- [ ] `7. 引导问题/问题列表.md`（9 视角 ~60-70 问，纯问题，带 `<!--tag:{视角}-->`）
- [ ] 同步 `guide_pool.py` 的 `GUIDE_POOL["knowledge-graph"]`（视角→组映射 intro/operation/data_relation/difficulty/rag，只放问题文本）→ 跑引导单测回归
...
| ⑤ 完善文档 9 节 | ⏳ | 为什么→方案→落地真相→追问→证据 | 1.0 前置校验，整篇禁切 |
| ⑥ 引导问题 + guide_pool | ⏳ | 问题列表 9 视角 + GUIDE_POOL 同步 | 只放问题不写答案 |
| ⑦ 切片 5 来源 | ⏳ | 代码双切/坑双切/引导逐问/语雀条目/OpenSpec### | 提示词 + 大模型语义分组 |
