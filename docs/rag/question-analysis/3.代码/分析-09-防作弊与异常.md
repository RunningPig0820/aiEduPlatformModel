...
> summary: 护栏保证答错不恭喜/不直接给答案/识别不出不硬猜/闲聊不乱收尾: 低置信挂起(空 labels→Java PENDING 走澄清卡)/闭集外放行(宁漏不误)/答非所问 type=concept 引导回题绝不 end/end 一致性护栏(答错被恭喜降级 concept)/换题短路 switch 不调 LLM/safety_flag 高危拦截; 结构化四段降级(function calling→JSON→正则→兜底 hint degraded)
> 权威度: 0.8
