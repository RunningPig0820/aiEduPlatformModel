...
> summary: 服务慢/挂的兜底: 识别/学科门统一慢修复(关思考+20s 超时+重试0 秒出), 业务判定吞异常降级空结果, 向量错误冒泡 Java 桥降级字符规则+原样落库不阻塞主链路, decide 流式降非流式四段, generate 空流兜底固定话术, COS 未配置懒加载 500 可降级, 向量是增强层主链路不依赖
> 权威度: 0.8
