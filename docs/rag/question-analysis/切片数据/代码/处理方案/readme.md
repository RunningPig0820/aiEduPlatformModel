...
- 提示词：`处理方案/提示词/切片-语义分组-提示词.md`（本目录下，替代硬编码脚本）
- 切法：每份文档按语义合并成 **3 块**——
  - **业务链路** = 业务描述与业务场景 + 职责 + 高层业务调用链
  - **代码事实** = 代码事实（含全部 ### 子节）+ 设计要点
  - **坑与对账** = 隐性坑与注意事项 + 对账要点
  - 丢弃：文档标题行/文件头元信息 + 已读代码清单
- 输出：每份 3 个 md 文件 → 本目录 `../` 下 `代码/`（`分析-0X-{主题}-{块名}.md`）
- 本次结果：10 份 → **30 块**
- 规则约束：只合并不改写正文（文件:行号/表格/mermaid/列表原样）；块内保留子标题层级；每块自包含；表格/mermaid 不切破
