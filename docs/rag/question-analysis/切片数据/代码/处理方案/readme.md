...
| **面试切片** | `提示词/切片-面试分组-提示词.md` | 业务与流程 / 架构与技术取舍 / 落地问题与复盘（过滤代码噪声+转述） | `处理方案/面试切片/` | 面试官提问/候选人介绍 |
...
- 本次结果：开发切片 30 块（`../代码/`）+ 面试切片 30 块（`处理方案/面试切片/`）
- 旧脚本方案（`slice_corpus.py` CODE_GROUP）保留在 `../脚本代码/` 作备选，不再作为主路径
