# design-java-rag-project-intro-assistant(白盒RAG项目介绍助手 Java侧设计)

> summary: Java侧白盒RAG项目介绍助手(rag-project-intro-assistant)设计定稿:学生页面提问走角色门→意图识别→多路召回→RRF重排→LLM生成全链路SSE事件白盒透传,含D1~D12核心决策与定稿契约D-A~D-E、交付编排M1-M8,复用tutoring网关与Python已有RAG链路,区别于08-21面试demo方向
> 权威度: 0.7
> 模块: rag-system
> COS路径: rag-source/rag-system/OpenSpec设计决策/design-java-rag-project-intro-assistant.md
> 类别：架构设计

## 文档说明

> 本文件为原始spec文档(design-java-rag-project-intro-assistant,08-25 Java侧白盒RAG助手)的RAG结构化重构版本。
> ⚠️重要提示:本文属于**设计阶段素材**,同时包含✅已落地(复用资产)、⚠️构想未实现(D1~D12设计决策/定稿契约/交付编排)、❓待决策(Open Questions)内容;业务真实实现请以权威度0.8的canonical真相源文档为准。本文件独立完整,内容不拆分到外部canonical文档。

### 业务背景与定位(Context)
> 状态：⚠️ 设计定位
> 检索摘要：白盒RAG项目介绍助手(Java侧)设计:学生页面提问走权限→意图→召回→RRF→生成全链路SSE透传,复用tutoring网关与Python RAG链路,区别08-21面试demo
...
### Goals
> 状态：⚠️ 设计目标
> 检索摘要：白盒RAG助手目标:角色硬门仅STUDENT、模块全放行低置信过滤、clarify澄清一轮、is_quoted用LCS、分层超时、trace_id计费、引导与评估复用run_eval链
...
> 状态：⚠️ 设计决策
> 检索摘要：意图识别用LLM结构化输出+关键词兜底:anchor模块级选语料池、locked_sections节级加权,输出ambiguous/candidates供clarify
...
> 状态：⚠️ 设计决策
> 检索摘要：切换判定收敛下一轮intent不做生成中切换:switch_detected发事件重置上下文,不掐在途流避免半截token白烧
...
> 状态：⚠️ 设计决策
> 检索摘要：四模块全放行意图无禁区拒答,范围门=检索置信度:RRF精排低于阈值0.75/0.5则低置信过滤固定话术,唯一拒答路径low_confidence
...
> 状态：⚠️ 设计决策
> 检索摘要：clarify澄清轮:歧义ambiguous且candidates≥2才问,固定话术0token最多一轮,默认当前功能,点选候选chip重发原问题
...
> 状态：⚠️ 设计决策
> 检索摘要：is_quoted用LCS最长公共子串硬匹配非LLM自述:8中/12英字符命中即引用,done后补发quoted_keys前端先灰后亮
...
> 状态：⚠️ 设计决策
> 检索摘要：计费透明:tokens_usage四字段+Java生成trace_id透传Python供断线补查,history前端传最近3轮不落库,turns只存Java Redis
...
> 状态：⚠️ 设计决策
> 检索摘要：问题提示=开始+结束引导以底座池为唯一来源,池内出题保证可答并引导到项目介绍/操作/数据关联/难点,RAG非展示页每轮必带
...
> 状态：⚠️ 设计决策
> 检索摘要：显式关闭对话close:中止在途流+会话置closed+返回会话累计token,区分断连取消异常路径,累计token归Java
...
### D-A. 模块 id 闭集(三端统一)
> 状态：⚠️ 定稿契约
> 检索摘要：模块id闭集三端统一:ai-tutoring/knowledge-graph/question-analysis/rag-system,弃用rag-project,语料按tags.module过滤
...
> 状态：⚠️ 定稿契约
> 检索摘要：permission事件携带trace_id由Java网关入口生成,流一开始前端即可取,断线补查不依赖done,Python无感
...
> 状态：⚠️ 定稿契约
> 检索摘要：问候识别:你好/Hi归category问候不触发clarify,走欢迎引导固定话术+引导建议指向四大方向,0生成token
...
### Migration Plan
> 状态：⚠️ 迁移规划
> 检索摘要：迁移落地:Python先行泛化query.py白盒链路新增ask SSE端点,Java网关RagAssistantController角色门+SSE中继,语料无迁移
