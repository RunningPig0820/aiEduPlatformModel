> summary: 本spec定义RAG项目介绍助手的护栏(guardrails)：AI答疑/知识图谱/题型分析/RAG四模块全放行无禁区+范围门低置信度过滤为唯一拒答路径、clarify澄清只一轮、is_quoted确定性引用硬匹配、模块可用性数据驱动、问题提示RAG常驻。
> 权威度: 0.7
> 模块: rag-system
> COS路径: rag-source/rag-system/OpenSpec设计决策/spec-java-rag-project-intro-assistant-guardrails.md
> 类别：业务流程
...
### 文档目的（Purpose）
> 状态：⚠️
> 检索摘要：模块全放行(无禁区)+范围门低置信度过滤唯一拒答路径、clarify澄清一轮、is_quoted硬匹配、模块可用性数据驱动；当前仅AI答疑有语料其余入库即自动可答。

模块全放行（AI答疑/知识图谱/题型分析/RAG 四模块无禁区）+ 范围门低置信度过滤（唯一拒答路径）、clarify 澄清轮、is_quoted 确定性引用硬匹配、模块可用性数据驱动（当前仅 AI答疑有语料，其余模块入库即自动可答）。

### 模块全放行与硬路由
> 状态：⚠️
> 检索摘要：AI答疑/知识图谱/题型分析/RAG四模块无禁区全放行，涉及系统架构/代码/部署/评测/接口设计强制路由RAG项目知识库，意图层不硬拒答。
...
### 范围门低置信度过滤（唯一拒答路径）
> 状态：⚠️
> 检索摘要：召回精排后综合分低于阈值(索引层0.75/源文档池0.5)返回固定话术，事件boundary+reason=low_confidence，唯一拒答路径不调generate。
...
### 固定话术不调 LLM
> 状态：⚠️
> 检索摘要：拒答话术(范围门低置信/超时降级)写死严禁调LLM，任何拒答分支0 token成本tokens_usage均为0。
...
### clarify 澄清轮
> 状态：⚠️
> 检索摘要：intent判ambiguous=true且候选≥2发clarify事件(固定话术+candidates+default)0token不计轮次；学生下一条仍模糊不二次澄清直接默认当前功能。
...
### is_quoted 确定性硬匹配
> 状态：⚠️
> 检索摘要：生成完成后对精排Top-K块text/summary与answer做最长公共子串匹配，连续8中文字符(或12英文)命中标记is_quoted=true，不依赖LLM自述。
...
### 模块可用性数据驱动
> 状态：⚠️
> 检索摘要：按语料实际存在与否决定模块可答：无语料模块正常召回但命中空/低置信走范围门低置信过滤，未来入库切片自动可答不改代码。
...
### 问题提示（开始引导 + 结束建议，RAG 常驻）
> 状态：⚠️
> 检索摘要：会话入口展示RAG定向引导(非SSE静态池0token)，每轮done.suggestions返回1~3条建议必含≥1条RAG方向，LLM失败静态池兜底。
...
### 切换上下文（switch 事件）
> 状态：⚠️
> 检索摘要：下一轮intent判switch_detected=true(前端current_project≠已锚定project或明确指向另一有语料模块)发switch事件重置上下文，不做生成中切换。
