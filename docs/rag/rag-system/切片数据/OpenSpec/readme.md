...
> 切片方式：**提示词 + 大模型按 ### 切**（脚本导出方案已废弃，2026-08-28 与 代码/坑档案/引导问题/语雀 对齐）。

## 一、源头（语料就绪）

- 源文档：`2.OpenSpec design 决策/design-python-project-intro-rag.md`（双池检索/页面锚定/两道门/token 真算/引导问题=索引层入口，D1~D11）+ `design-python-rag-eval-agent.md`（评测 agent hit@k + answer_quality，D1~D7）（2 份 RAG 版，权威 0.7，素材溯源库；`###` 小节数待生成）
- 原始草稿归档：`2.OpenSpec design 决策/proposal-*.md`（2 份 proposal，双轨素材，不进池）
- 生成提示词：`2.OpenSpec design 决策/处理方案/提示词/`（双轨：design 自身成文进池 + 收敛事实折入语雀 canonical）

## 二、切片（提示词 + 大模型按 ### 切）

> 切片方式 2026-08-28 定为提示词 + 大模型：一个 `###` 小节 = 一个 chunk = 一个文件（决策 D#/背景/目标非目标/风险与权衡/迁移计划/开放问题）。提示词：`处理方案/提示词/OpenSpec-切片-提示词.md`。

- 粒度：决策 `### D#` → `design-{change}-D{n}-{短名}.md`（编号归一化 D{#}）；背景→Context、目标非目标→Goals-Non-Goals、风险/迁移/开放问题各自成块
