...
| 源 `###` 小节 | 输出文件名 |
|---|---|
| `## Context` | `design-{change}-Context.md` |
| `## Goals / Non-Goals` | `design-{change}-Goals-Non-Goals.md` |
| `### D#：...` | `design-{change}-D{编号}-{短名}.md`（编号**统一归一化 `D{#}`**：决策 #/Decision # → D#） |
| `## Risks / Trade-offs` | `design-{change}-风险与权衡.md` |
| `## Migration Plan` | `design-{change}-迁移计划.md` |
| `## Open Questions`（含"全部已决"类） | `design-{change}-开放问题.md` |
| 顶层 `# ==== 分节 ====`（如存在） | **不是 chunk**——章节分隔线，其下 `###` 正常切，不单独成文件 |

`{change}` = 源文件名去 `design-` 前缀（如 `python-project-intro-rag`）；`{短名}` = 小节标题去编号冒号后短名。**文件名全部小写，禁止大写/空格/特殊符号**。

## 硬性规则
1. **一个 ### 一文件，自包含**：单块能独立回答对应设计问题，不依赖其它块。
2. **原文保真 100%**：决策正文/属性表/表格/流程图文本/代码块**原样保留，不转述不改写**；**特别完整保留 风险与权衡/迁移计划/开放问题**（素材溯源价值所在）；只删 文档说明。
3. **不写状态，检索摘要必须有**：切片头/正文**不写 `> 状态`**（✅/⚠️/❓ 已移除）；**每块正文必须带 `> 检索摘要：`**（源小节检索摘要原样完整；缺失补写 1-2 句富含核心实体与动作）——检索摘要即召回锚点。素材层未落地/待决语义由 `authority=0.7 + source=OpenSpec` 层规则兜底。**不自动生成 WARNING**（见定位⚠️）。
...
5. **头部精简（机器元信息摄入时推导，md 头不写）**：切片 md 头只保留 summary/权威度/模块/COS路径/类别/状态 6 行；`entry_id`（←文件名段：{change}-D{#}/Context/Goals-Non-Goals/风险与权衡/迁移计划/开放问题，超长子块追加 `-2/-3`）、`source_doc`（←文件名前缀 design-{change}.md）、`status tag`（← `> 状态` 映射）由 QT⑥⑦ 摄入时推导，不在切片头冗余。
...
## 输出格式（每个 ### 一个 md，输出 `docs/rag/rag-system/切片数据/OpenSpec/切片/`）
...
> 权威度: 0.7
> 模块: rag-system
> COS路径: rag-slices/rag-system/OpenSpec/{文件名}.md
> 类别：{9 视角闭集单值}
...
> 双池/页面锚定/两道门/降级矩阵/路由/打分 → 架构设计；
> token 真算/usage/成本/向量桶 → 数据存储；
> 索引层 QA 池 + 源文档池/引导问题=索引层入口/锚定 → 数据关联；
> 接口契约/降级/重试/边界 → 开发难点；
> 前端展示/交互/引导渲染/演示 → 操作流程；
> 设计原则/建议项/范围降级 → 业务视角；
> 背景/目标非目标 → 项目介绍；
> 开放问题 → 未来演进；
...
5. ✅ 无 `> 状态` 行；每块正文带 `> 检索摘要：`（源检索摘要完整/缺失补写）。
6. ✅ 决策正文/风险/迁移/开放问题 100% 保留，未删减业务信息；正文**保留 `> 检索摘要`**（原文保真，召回锚点）。
...
## 使用提示
1. **一次处理 1 份文档**（避免上下文过长漏小节）；2 份按 输入表顺序。
2. 产出前清空 `切片数据/OpenSpec/切片/` 旧切片（保留 readme.md）。
3. **QT⑥⑦ 机器元信息推导（下游落地约定）**：`slice_corpus.py` 从文件名+头部推导，md 头不写机器字段——`entry_id` ← 文件名段（`design-{change}-D{n}-*`→{change}-D{n}、`design-{change}-Context.md`→Context、超长子块 `-2/-3` 后缀）；`source_doc` ← 文件名前缀（`design-{change}-*`→design-{change}.md）；`source/authority/module` ← 层配置（source=OpenSpec、authority=0.7、module=rag-system）。**无 status tag**（状态已删）——检索侧 `meta.authority == 0.7 && meta.source == "OpenSpec"` 识别素材层，引用必须提示核对代码。
