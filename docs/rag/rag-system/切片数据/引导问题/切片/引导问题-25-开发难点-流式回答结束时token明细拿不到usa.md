...
- **⚠️ 半修（诚实口径）**：白盒流式已修；1.6C `/query` 端点仍丢（`generate` 不带 `return_usage`、`RAGQueryResponse` 无 usage 字段）、embedding 侧仍丢（`vector_store.embed()` 未抓 `resp.usage`）——面试表述按"白盒链路真采、1.6C/embedding 未接"为准（依据：坑档案 K6 / 分析-07）。
