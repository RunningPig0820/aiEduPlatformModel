...
### 1.1 测试目标
验证 `curriculum-kp-extraction` 模块的所有业务场景，确保 OCR 识别、LLM 提取、对比分析功能的正确性。
...
| OCR-009 | 边界-进度指示 | 大文件处理 | 50页 PDF | 显示进度百分比 |
...
| 用例编号 | 场景描述 | 前置条件 | 输入 | 预期结果 |
|---------|---------|---------|------|---------|
| TTL-001 | 正常生成-TTL 文件 | 知识点有效 | 知识点列表 | 生成 curriculum_kps.ttl |
...
# 跳过需要 GPU 的测试
pytest tests/curriculum/ -v -m "not gpu"
